...
def test_ticket_validations():
    ...
    # Test with allowed ticket
    allowed_ticket = config.ALLOWED_PARENT_TICKETS[0]
    print(f"Testing with allowed ticket: {allowed_ticket}")
    try:
        # Just validate the ticket - don't actually create a work item
        result = work_items_api.is_parent_ticket_allowed(allowed_ticket)
        print(f"Validation result: {result}")
        print("✅ Validation passed for allowed ticket")
    except Exception as e:
        print(f"❌ Error: {str(e)}")

    # Test with non-allowed ticket
    ...
    # For each allowed ticket, check if it's resolved
    for ticket in config.ALLOWED_PARENT_TICKETS:
        ...
        try:
            ...
            if is_resolved:
                ...
                try:
                    ...
                    # This should pass if the ticket is not resolved and is in the allowed list
